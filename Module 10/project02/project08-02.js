...
function ball(size) {
   ...
   ball.prototype.MoveWithin = function(conatiner) {
      let ballTop = this.yPos;
      let ballLeft = this.xPos;
      let ballBottom = this.yPos + this.radius;
      let ballRight = this.xPos + this.radius;

      if(ballTop < 0 || ballBottom > conatiner.height){
         conatiner.yPos += this.yVelocity;
         this.yVelocity =- this.yVelocity;
      } else if (ballLeft < 0 || ballRight > conatiner.width) {
         conatiner.xPos += this.xVelocity;
