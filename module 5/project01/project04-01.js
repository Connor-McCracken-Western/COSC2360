...
function calcTotal() {
   ...
   msgBox.innerHTML = "";  // Erase any warnings in the message box
      ...
      try {
          if (!(wgtBox.value > 0)) throw "!! Enter a poitive weight";
          totalCost += wgtBox.value * COST_PER_LB;
      } catch(error) {
         document.getElementById("msgBox").innerHTML = ("!! Enter a positive weight");
      }

      
      try {
          if (!(distBox.value > 0)) throw "!! Enter a poitive mileage";
          totalCost += distBox.value * COST_PER_MILE;
      } catch(error) {
         document.getElementById("msgBox").innerHTML = ("!! Enter a positive mileage");
      }
